...
def axes_setting(ax, show = False, datashape = (512, 512), pixsize = 0.2):
	...
	if not(show):
		...
		# print(1)
		lon.set_axislabel('Offset', minpad = 0.1)
		lat.set_axislabel('Offset', minpad = 0.1)
		lon.set_ticklabel_visible(False)
		lat.set_ticklabel_visible(False)
		# lon.set_ticklabel_visible(False)
...
sample_info = pd.DataFrame(
    {
        "source_name": ["DR21OH"],
        "other_name": ["DR21OH"],
        "continuum_file": ["d21.cont.lms.fits"],
        "outflow_index0": [15],
		"outflow_angle0_red":[np.nan],
		"outflow_angle0_blue":[110],
		"disk_angle0":pd_value,
		"disk_pvsigma0": pd_value,
		"kepler_central_mass": pd_value,
		"outflow_index1": [16],
		"outflow_angle1_red":[60],
		"outflow_angle1_blue":[-120]
    }
)
...
for source_name, subplot_index in zip(source_list, range(len(source_list))):
	...
	conden_pos_list = [condenpos(conden_info) for conden_info in conden_info_list]
	...
	fig0.savefig("SMA1.3mmCont_DR21OH.pdf", dpi = 360, format ="pdf", bbox_inches = 'tight')

	# Read outflow data(including mosaic mode)
	
	outflow_list = [
		outflow_path + "/Red_lobe/cygx"+ source_name.lower()+"p023456_CO_2_1.red.fits",
		outflow_path + "/Blue_lobe/cygx"+ source_name.lower()+"p023456_CO_2_1.blue.fits",
		outflow_path + "/Red_lobe/cygx"+ source_name.lower()+"p789_CO_2_1.red.fits",
		outflow_path + "/Blue_lobe/cygx"+ source_name.lower()+"p789_CO_2_1.blue.fits",
        outflow_path + "/Red_lobe/cygx"+ source_name.lower()+"p101112_CO_2_1.red.fits",
		outflow_path + "/Blue_lobe/cygx"+ source_name.lower()+"p101112_CO_2_1.blue.fits",
	]
	...
	if os.path.exists(outflow_list[4]):
		color_list.append('red')
		linestyle_list.append("solid")
		alpha_list.append(0.8)
		filename_list.append(outflow_list[4])

	if os.path.exists(outflow_list[5]):
		color_list.append('blue')
		linestyle_list.append("solid")
		alpha_list.append(0.8)
		filename_list.append(outflow_list[5])
	...
	for filename, color, linestyle, alpha in zip(filename_list, color_list, linestyle_list, alpha_list):
		...
		upperpercent = 0.95; lowerpercent = 0.35
		num_bin = 5
		rms_level = [i*vmax for i in np.linspace(lowerpercent, upperpercent, num_bin)]

		cntr = ax.contour(imgdata, transform= ax.get_transform(outflow_wcs), levels = rms_level, colors = color, linewidths = 0.2, linestyles = linestyle, alpha = alpha)
		...
		if not(np.isnan(outflow_index)):
			...
			for color, angle in zip(arrow_color_list, angle_list):
				if not(np.isnan(angle)):
					arrow_plot(ax, angle, color, outflow_index,target_name, conden_cata)

	# plt.savefig("CO/%s_CO_outflow.pdf"%source_name, format="pdf", dpi = 360)
	

fig0.subplots_adjust(wspace=0.0, hspace=0.0)
fig0.savefig("outflowVScontinuum_DR21OH.pdf", dpi = 360, format ="pdf", bbox_inches = 'tight')
